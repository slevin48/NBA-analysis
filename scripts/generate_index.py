...
def _logo_block(tricode: str) -> str:
    art = ASCII_LOGOS.get(tricode.upper())
    if not art:
        art = f"""
┌─────┐
│ {tricode.upper():^3} │
...
"""
    ...
    return f"<pre class='logo' aria-label='{escape(tricode)} logo'>{escape(art)}</pre>"
...
def render_html(game_date: str, games: List[Dict[str, Any]]) -> str:
    ...
    for game in games:
        ...
        cards.append(
            ...
            f"<div class='teams'>"
            f"<div class='team away'><div class='team-meta'>{_logo_block(away['tricode'])}" \
            f"<div class='team-text'><p class='team-name'>{escape(away['city'])} {escape(away['name'])} ({away['tricode']})" \
            f"<span class='record'>{away['wins']}-{away['losses']}</span></p></div></div>" \
            f"<p class='score'>{away['score']}</p></div>"
            f"<div class='team home'><div class='team-meta'>{_logo_block(home['tricode'])}" \
            f"<div class='team-text'><p class='team-name'>{escape(home['city'])} {escape(home['name'])} ({home['tricode']})" \
            f"<span class='record'>{home['wins']}-{home['losses']}</span></p></div></div>" \
            f"<p class='score'>{home['score']}</p></div>"
...
<head>
  ...
  <style>
    ...
    * {{ box-sizing: border-box; }}
    ...
    .team {{ display: flex; justify-content: space-between; align-items: center; padding: 0.4rem 0.25rem; gap: 0.75rem; }}
    .team-meta {{ display: flex; align-items: center; gap: 0.75rem; }}
    .team-text {{ display: flex; flex-direction: column; gap: 0.1rem; }}
    .team-name {{ margin: 0; font-weight: 600; }}
    .record {{ color: var(--muted); font-weight: 400; font-size: 0.85rem; margin-left: 0.35rem; }}
    .logo {{ margin: 0; background: rgba(255,255,255,0.03); border: 1px solid rgba(102, 252, 241, 0.2); border-radius: 8px; padding: 0.35rem 0.5rem; font-family: 'SFMono-Regular', Consolas, 'Liberation Mono', Menlo, monospace; line-height: 1.1; color: var(--accent); min-width: 6ch; }}
